...
// Função mais cuidadosa para remover console.log
function removeConsoleLogsCarefully(content) {
  let modifiedContent = content;
  let removedCount = 0;

  // Padrões mais específicos e seguros
  const patterns = [
    // console.log simples com ponto e vírgula no final da linha
    /^\s*console\.log\([^)]*\);\s*$/gm,
    // console.log com múltiplas linhas (mais cuidadoso)
...
function processFile(filePath) {
  try {
    ...
    const result = removeConsoleLogsCarefully(content);

    if (result.hasChanges) {
      // Criar backup
      const backupPath = `${filePath}.backup-careful-${Date.now()}`;
...
// Função principal
function main() {
  console.log('🧹 Iniciando remoção CUIDADOSA de console.log...\n');
  ...
  if (changedCount > 0) {
    console.log(`\n✅ Remoção CUIDADOSA de console.log concluída!`);
    console.log(
...
module.exports = { removeConsoleLogsCarefully, processFile, findSourceFiles };
