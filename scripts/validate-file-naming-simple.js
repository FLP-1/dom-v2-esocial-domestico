...
/**
 * Script simples para validar nomenclatura de arquivos
...
function validateFile(filePath) {
  ...
  if (filePath.includes('/components/')) {
    ...
    if (fileName.startsWith('_')) {
      // Arquivos especiais do Next.js (_app, _document) são permitidos
      return errors;
    }
    // Páginas podem ser camelCase ou kebab-case (mais flexível)
    if (!/^src\/pages\/[a-z][a-zA-Z0-9-]*\.tsx$/.test(relativePath)) {
      errors.push('🚫 Página deve estar em camelCase.tsx ou kebab-case.tsx');
    }
