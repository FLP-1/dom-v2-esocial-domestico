...
export default async function handler(
  ...
  try {
    console.log('🔴 === TESTE DIRETO XML DEFINITIVO ===');
    console.log('🎯 Testando EXATAMENTE a estrutura fornecida!');
  ...
  <soapenv:Body>
    <ConsultarLoteEventos xmlns="http://www.esocial.gov.br/servicos/empregador/lote/eventos/envio/consulta/retornoProcessamento/v1_1_0">
      ...
      try {
        ...
        console.log(`Status: ${response.status} (${tempoResposta}ms)`);
        console.log(
          `Progresso de 403: ${response.status !== 403 ? 'SIM' : 'NÃO'}`
        );

        if (response.status === 200) {
          console.log('🎉 SUCESSO! Solução definitiva funcionou!');
          break;
        }
      } catch (error) {
        resultados.push({
