...
class ESocialSoapServerService {
  ...
  async initialize(certPath: string, certPassword: string): Promise<void> {
    try {
      ...
      console.log(
        `✅ Cliente SOAP eSocial Doméstico inicializado para ${this.environment}`
      );
  ...
  async consultarEmpregador(
    ...
    console.log(
      `🔍 Consultando empregador: ${cpfCnpj} no ambiente ${this.environment} (Modo Simulação Inteligente)`
    );
...
export default async function handler(
  ...
  try {
    ...
    console.log(`🔍 Debug - isRealMode: ${isRealMode}, fonte: ${fonte}`);
