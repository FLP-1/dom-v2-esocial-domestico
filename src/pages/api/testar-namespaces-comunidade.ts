...
export default async function handler(
  ...
  try {
    console.log('🔍 === TESTANDO NAMESPACES DA COMUNIDADE ===');
    console.log('Traduzindo soluções C#, PHP e Delphi para TypeScript');
    ...
    for (const variacao of variacoesNamespace) {
      ...
      console.log(`📚 Fonte: ${variacao.fonte}`);
      console.log(`🔗 Namespace: ${variacao.namespace}`);
  ...
  <soap:Body>
    <ConsultarLoteEventos xmlns="${variacao.namespace}">
      <consulta>
        ...
        console.log(`📤 Enviando com namespace: ${variacao.versao}`);
        ...
        console.log(`${variacao.fonte} - Status: ${response.status}`);

        if (response.status === 200 && analise.funcionou) {
          console.log(`✅ SUCESSO! Namespace ${variacao.versao} funcionou!`);

          // Salvar solução funcional
          await salvarSolucaoFuncional(variacao, xmlCSharp, headers);
          break;
        } else if (response.status !== 403) {
          console.log(`⚠️ Progresso! Status ${response.status} (não é 403)`);
        }
      } catch (error) {
        resultados.push({
          nome: variacao.nome,
...
async function salvarSolucaoFuncional(
  ...
  try {
    ...
    console.log(
      '💾 Solução funcional salva em: solucao-namespace-funcional.json'
    );
  } catch (error) {
    console.log('⚠️ Não foi possível salvar solução:', error.message);
  }
}
