...
export default async function handler(
  ...
  try {
    ...
    // Teste 1: Conectividade SSL com endpoints corretos
    try {
      ...
      const response = await axios.get(testUrls[environment], {
      ...
      results.tests.push({
        test: 'conectividade_ssl_endpoints_corretos',
        status: 'SUCCESS',
        responseTime,
        success: true,
        statusCode: response.status,
        message: 'Conectividade SSL estabelecida com endpoints corretos',
        url: testUrls[environment],
      });
    } catch (error) {
      results.tests.push({
        test: 'conectividade_ssl_endpoints_corretos',
        ...
        url:
          environment === 'homologacao'
            ? 'https://pre-esocial.serpro.gov.br'
            : 'https://webservices.esocial.gov.br',
      });
    }

    // Teste 2: Consulta de empregador com endpoints corretos
    try {
      const consultaResult = await esocialSoap.consultarEmpregador();
      results.tests.push({
        test: 'consulta_empregador_endpoints_corretos',
        ...
        message: consultaResult.success
          ? 'Consulta de empregador funcionando com endpoints corretos'
          : 'Erro na consulta com endpoints corretos',
      });
    } catch (error) {
      results.tests.push({
        test: 'consulta_empregador_endpoints_corretos',
    ...
    // Teste 3: Envio S-1000 com endpoints corretos
    try {
      const s1000Result = await esocialSoap.enviarS1000();
      results.tests.push({
        test: 'envio_s1000_endpoints_corretos',
        ...
        message: s1000Result.success
          ? 'S-1000 enviado com sucesso usando endpoints corretos'
          : 'Erro no envio S-1000 com endpoints corretos',
      });
    } catch (error) {
      results.tests.push({
        test: 'envio_s1000_endpoints_corretos',
    ...
    const summary = {
      ...
      recommendation:
        failureCount === 0
          ? 'Sistema funcionando perfeitamente com endpoints corretos!'
          : 'Verificar erros específicos antes de usar em produção',
    };
    ...
    console.error('Erro no teste de endpoints corrigidos:', error);
