...
export default async function handler(
  ...
  try {
    console.log('🎯 === TESTE FINAL S-1.3 OFICIAL ===');
    console.log(
      'Baseado nas informações de que S-1.3 está ativa e funcionando'
    );
  ...
  <soap:Body>
    ...
    for (const estrutura of estruturasOficiais) {
      ...
      try {
        ...
        console.log(
          `Status: ${response.status} | Sucesso: ${response.status === 200}`
        );

        if (response.status === 200) {
          console.log('✅ SUCESSO! Estrutura XML funcionou!');
