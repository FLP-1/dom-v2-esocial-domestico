      ...
      diagnostico.testes.push({
        ...
        detalhes: {
          ...
          conectividade_basica:
            wsdlPing && endpointPing ? 'OK' : 'PING BLOQUEADO (NORMAL)',
          observacao:
            'Servidores de produção frequentemente bloqueiam ping por segurança',
        },
      });
    } catch (error) {
      diagnostico.testes.push({
    ...
    try {
      ...
      diagnostico.testes.push({
        ...
        detalhes: {
          ...
          observacao: consultaResult.success
            ? ''
            : 'CPF precisa ser cadastrado via portal oficial do eSocial',
        },
      });
    } catch (error) {
      diagnostico.testes.push({
