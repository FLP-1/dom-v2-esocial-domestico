...
export default async function handler(
  ...
  if (req.method === 'POST') {
    try {
      ...
      console.log(`📄 API Upload - Categoria: ${category}`);
      console.log(`📄 API Upload - Nome: ${name}`);
      console.log(`📄 API Upload - Usuário: ${userId}`);
      console.log(
        `📄 API Upload - CPF: ${cpf ? '***' + cpf.slice(-3) : 'N/A'}`
      );
      ...
      console.log(`✅ API Upload - Documento criado: ${documentId}`);
