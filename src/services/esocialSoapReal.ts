...
<soap:Envelope
  ...
  private generateConsultarEmpregadosXML(): string {
    console.log('🏛️ Usando API oficial de Qualificação Cadastral');
  ...
  private signXML(xml: string): string {
    ...
    console.log(
      '🔐 XML será assinado digitalmente (implementação simplificada)'
    );
    return xml;
  }
  ...
  private async makeSoapRequest(
    ...
    try {
      ...
      console.log('🔧 DEBUG SOAP - Ambiente:', this.config.environment);
      console.log('🔧 DEBUG SOAP - URL:', urls.consultaTrabalhador);
      console.log('🔧 DEBUG SOAP - Action:', action);
      ...
      if (response.status === 500 && responseData) {
        console.log('🔍 CONTEÚDO SOAP (erro 500):');
  ...
  async consultarDadosEmpregador(): Promise<ESocialResponse> {
    try {
      console.log('🏢 Consultando dados cadastrais do empregador...');
...
<soap:Envelope
  ...
  async testarConsultaEmpregador(): Promise<ESocialResponse> {
    try {
      console.log('🧪 Testando consulta básica de empregador...');
...
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/"
  ...
  private async makeSoapRequestEmpregador(
    ...
    try {
      ...
      console.log('🔧 DEBUG EMPREGADOR - Ambiente:', this.config.environment);
      console.log('🔧 DEBUG EMPREGADOR - URL:', urls.consultaEmpregador);
      console.log('🔧 DEBUG EMPREGADOR - Action:', action);
      ...
      console.log('📥 Resposta SOAP EMPREGADOR recebida:', {
        status: response.status,
        headers: response.headers,
        dataLength: response.data?.length || 0,
      });
  ...
  private parseEmpregadorResponse(xmlData: string): any {
    console.log('📋 Parseando dados do empregador...');
    ...
    try {
      ...
      console.log('⚠️ Erro ao parsear XML, usando dados padrão');
      return {
        cpf: this.config.companyId,
  ...
  async consultarEventosPorFiltro(): Promise<ESocialResponse> {
    try {
      console.log('📋 Consultando eventos por filtro S-2200...');
  ...
  async consultarQualificacaoCadastral(
    ...
    try {
      console.log(
        `🏛️ Consultando Qualificação Cadastral OFICIAL para CPF: ${cpfTrabalhador}`
      );
  ...
  async consultarPorCpfTrabalhador(
    ...
    try {
      console.log(`👤 Consultando trabalhador por CPF: ${cpfTrabalhador}`);
  ...
  private async makeSoapRequestEventos(
    ...
    try {
      ...
      console.log('🔧 DEBUG EVENTOS - URL:', urlEventos);
      ...
      console.log('📥 Resposta EVENTOS recebida:', {
        status: response.status,
        dataLength: response.data?.length || 0,
      });
  ...
  private async makeSoapRequestQualificacao(
    ...
    try {
      ...
      console.log('🔧 DEBUG QUALIFICAÇÃO - URL:', urlQualificacao);
      ...
      console.log('📥 Resposta QUALIFICAÇÃO recebida:', {
        status: response.status,
        dataLength: response.data?.length || 0,
      });
      ...
      if (response.status !== 200) {
        console.log('🔍 Resposta detalhada:', response.data);
      }
  ...
  private parseQualificacaoCadastralResponse(
    ...
    console.log(`👤 Parseando Qualificação Cadastral para CPF: ${cpf}`);
    ...
    try {
      ...
      console.log(
        '⚠️ Erro ao parsear Qualificação Cadastral, usando dados das imagens'
      );
      return {
  ...
  private parseEventosResponse(xmlData: string): any[] {
    console.log('📋 Parseando eventos S-2200...');
  ...
  private parseTrabalhadorResponse(xmlData: string, cpf: string): any {
    console.log(`👤 Parseando dados do trabalhador CPF: ${cpf}`);
  ...
  async enviarEventoS2200(dadosEmpregado: any): Promise<ESocialResponse> {
    try {
      console.log('📤 Enviando evento S-2200 (Cadastramento de Empregado)...');
      console.log('👤 Dados:', JSON.stringify(dadosEmpregado, null, 2));
      ...
      if (response.success) {
        console.log('✅ S-2200 enviado com sucesso!');
...
<soap:Envelope
  ...
  private async makeSoapRequestEnvio(
    ...
    try {
      ...
      console.log('🔧 DEBUG ENVIO - URL:', urls.endpoint);
      ...
      console.log('📥 Resposta S-2200 recebida:', {
        status: response.status,
        dataLength: response.data?.length || 0,
      });
  ...
  private parseS2200Response(xmlData: string): any {
    try {
      console.log('📋 Parseando resposta do S-2200...');
      ...
      console.log('✅ Dados extraídos da resposta S-2200');
      return dadosExtraidos;
    } catch (error) {
      console.log('⚠️ Erro ao parsear resposta S-2200');
      return {
        erro: error instanceof Error ? error.message : 'Erro no parse',
  ...
  async consultarLotePorProtocolo(protocolo: string): Promise<ESocialResponse> {
    try {
      console.log(`🔍 Consultando lote por protocolo: ${protocolo}`);
...
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/"
  ...
  private async makeSoapRequestConsulta(
    ...
    try {
      ...
      console.log('🔧 DEBUG CONSULTA - URL:', urlConsulta);
      ...
      console.log('📥 Resposta CONSULTA recebida:', {
        status: response.status,
        dataLength: response.data?.length || 0,
      });
      ...
      if (response.status !== 200) {
        ...
        if (response.data && typeof response.data === 'string') {
          ...
          if (faultMatch) {
            console.log('🚨 SOAP FAULT ENCONTRADO:', faultMatch[1]);
          }
          ...
          if (faultStringMatch) {
            console.log('🚨 FAULT STRING:', faultStringMatch[1]);
          }
          ...
          if (faultCodeMatch) {
            console.log('🚨 FAULT CODE:', faultCodeMatch[1]);
          }
        }
      }
