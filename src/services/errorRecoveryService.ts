...
class ErrorRecoveryService {
  ...
  private async retryOperation(_errorInfo: ErrorInfo): Promise<void> {
    ...
    // console.log(`🔄 Tentativa ${errorInfo.tentativas} para erro ${errorInfo.id}`);
  }
  ...
  private async executeFallback(errorInfo: ErrorInfo): Promise<void> {
    ...
    switch (errorInfo.tipo) {
      case 'network':
        ...
        // console.log('🌐 Executando fallback de rede');
        break;
      case 'authentication':
        // Tentar reautenticação
        // console.log('🔐 Executando fallback de autenticação');
        break;
      case 'validation':
        // Usar dados padrão
        // console.log('✅ Executando fallback de validação');
        break;
      default:
      // console.log('🔄 Executando fallback genérico');
    }
  }
  ...
  private async sendErrorNotification(_errorInfo: ErrorInfo): Promise<void> {
    ...
    // console.log(`📢 Notificação de erro enviada: ${errorInfo.mensagem}`);
  }

  // Executar rollback
  private async executeRollback(_errorInfo: ErrorInfo): Promise<void> {
    // Implementar lógica de rollback
    // console.log(`⏪ Executando rollback para erro ${errorInfo.id}`);
  }
