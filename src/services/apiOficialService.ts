...
export class ApiOficialService {
  ...
  async consultarPortalTransparencia(
    ...
    try {
      console.log(`🏛️ Consultando Portal da Transparência para CPF: ${cpf}`);
      ...
      if (response.ok) {
        ...
        if (data && data.length > 0) {
          console.log('✅ Dados encontrados no Portal da Transparência!');
  ...
  async consultarCNIS(
    ...
    try {
      console.log(`🏛️ Consultando CNIS (Relação Trabalhista) para CPF: ${cpf}`);
      ...
      if (response.ok) {
        const data = await response.json();
        console.log('✅ Dados obtidos do CNIS!');
  ...
  async consultarDataprev(
    ...
    try {
      console.log(
        `🏛️ Consultando Dataprev (Qualificação Cadastral) para CPF: ${cpf}`
      );
      ...
      if (response.ok) {
        const data = await response.json();
        console.log('✅ Dados obtidos do Dataprev!');
  ...
  async consultarViaESocial(
    ...
    try {
      console.log(`🏛️ Consultando via eSocial (método que já funciona)`);
      ...
      if (response.ok) {
        ...
        if (data.success) {
          console.log('✅ Dados obtidos via eSocial!');
          return {
            success: true,
            data: {
